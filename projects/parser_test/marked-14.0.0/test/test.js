...
//   const gfm = JSON.parse(fs.readFileSync('../../datasets/gfm_ext.spec.json', 'utf8'));
  ...
  for(const item of commonmark){
    commonmarkTotal += 1;
    let result = marked.parse(item.markdown);
    if(result == item.html){
      commonmarkPass += 1;
    }
  }
...
const testPerformance = async()=>{
  ...
  const bigMDPath = '../../../datasets/big_testdata.md';
  const smallMDPath = '../../../datasets/small_testdata.md';
  let bigMD;
  let smallMD;
  try {
  ...
  const parsePerformance = (md,type)=>{
    ...
    for(let _=0; _<cnt; _++){
      ...
      let result = marked.parse(md);
      const end = process.hrtime.bigint();
      const second = Number(end - start);
      totalSecond += second;
    }
    console.log(`parse ${type}耗时: ${(totalSecond / 1e6 / cnt).toFixed(2)} 毫秒`);
  }
